...
namespace ReportOverviewApp.Models.WidgetModels
{
    ...
    {
        private static Func<Report, bool> Before(DateTime date) => r => r.CurrentDeadline() <= date;
        private static Func<Report, bool> After(DateTime date) => r => r.CurrentDeadline() >= date;
